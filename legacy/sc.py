...
from legacy.be import Noise_generator,Delta_repulsion,find_closest_element
...
def calculate_choice_and_rewards(x, gamma, sigma_update, s_hat, categories, no_response, seed):
...
def SC_model(func, x, y, s, s_hat, categories, sigma_noise, A_repulsion, gamma, sigma_update, no_response,
             Not_Blockstart, seed, mode):
                 
    """
    # Inputs:
    # func: A function to compute update matrix based on model choices and rewards.
    # x: Array representing the stimulus space.
    # y: A placeholder (not used in this function directly, but passed for future use).
    # s: Stimulus array for the current trials.
    ...
    # no_response: Boolean array indicating whether the subject has responded or not in each trial.
    # Not_Blockstart: Indicator for block start in trials.
    ...
    if mode == 'simulated':
        ...
        chooseB, rewards, yA, yB = calculate_choice_and_rewards(x, gamma, sigma_update, s_hat, categories, no_response,
                                                            seed)
    ...
    # Reverse the order of the rows of the matrices (in order to match the format of plots in the pre-print).
