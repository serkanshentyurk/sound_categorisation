...
def resolve_sigma(
    ...
    sigma = float(compute_stat(condition, ['sigma']).pooled['sigma'])
...
def compute_adaptation(
    ...
    *,
    ...
    baseline = compute_stat(expert, stat_names).pooled.to_dict()
    ...
    rolled = compute_rolling_stats(target, stat_names, per_session=(mode == 'per_session'),
                                   window=window, step=step)

    def _block(session):
        """(centres, {stat: values}) for one session label of the tidy frame."""
        c = rolled.curves[rolled.curves['session'] == session]
        centres = np.asarray(sorted(dict.fromkeys(c['trial'])), dtype=float)
        vals = {}
        for sn in stat_names:
            cs = c[c['stat'] == sn].set_index('trial')['value']
            vals[sn] = cs.reindex(centres).to_numpy(dtype=float)
        return centres, vals
    ...
    if mode == 'pooled':
        centres, raw = _block('pooled')
        vals = _standardise(raw)
        result['curve'] = {'trials': centres, 'values': vals, 'n_trials': rolled.n_trials('pooled')}
        result['rows'] = _collapse_rows(centres, vals, stat_names)
        return result
    ...
    for sess in target:
        ...
        centres, raw = _block(sid)
        vals = _standardise(raw)
        si = switch_of.get(sid, np.nan)
        entries.append({
            'session_id': sid, 'session_idx': getattr(sess, 'session_idx', None),
            'switch_index': si, 'session_type': getattr(sess, 'session_type', ''),
            'trials': centres, 'values': vals,
            'n_trials': rolled.n_trials(sid),
        })
        for r in _collapse_rows(centres, vals, stat_names):
