...
(across animals), never the session level — 4–6 sessions cannot support a test.
    ...
    opto = compute_stat(cond_opto,     stats, per_session=True, animal_id='SS15')
    ctrl = compute_stat(cond_non_opto, stats, per_session=True, animal_id='SS15')
...
``sessions`` frame of a ``compute_stat(per_session=True)`` result (columns: animal,
...
def plot_session_stats_single(
    ...
    Args:
        groups:      ``{group_label: sessions_frame}``. Each frame is the
                     ``sessions`` frame of ``compute_stat(per_session=True)``.
