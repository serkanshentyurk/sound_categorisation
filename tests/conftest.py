...
def _make_session(session_idx, base_date, trials, distribution='Uniform',
                  stage='Full_Task_Cont', masking=False):
...
def synthetic_opto_animal(rng):
    ...
    phases = (
        [(i, 'Uniform', 0.0, False) for i in range(5)] +
        [(i, 'Uniform', 0.3, True) for i in range(5, 7)] +
        [(i, 'Uniform', 0.3, False) for i in range(7, 12)] +
        [(i, 'Uniform', 0.0, False) for i in range(12, 14)] +
        [(i, 'Asym_Right', 0.3, False) for i in range(14, 19)] +
        [(i, 'Asym_Right', 0.0, False) for i in range(19, 24)]
    )

    sessions = []
    base_date = date(2026, 3, 1)

    for idx, dist, opto_frac, is_masking in phases:
        trials = _make_trial_data(350, rng, noise=0.20, opto_frac=opto_frac)
        sess = _make_session(
            idx, base_date, trials,
            distribution=dist, masking=is_masking,
        )
        sessions.append(sess)
