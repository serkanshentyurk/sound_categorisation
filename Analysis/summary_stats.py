...
def compute_summary_stats_per_session(
    session_data: List[Dict[str, np.ndarray]],
    stat_names: Optional[List[str]] = None,
    return_dict: bool = False,
) -> List[Union[np.ndarray, Dict]]:
    """
    Compute summary statistics for a list of variable-length sessions.
    
    Use this for real behavioural data where sessions have different
    trial counts. For SBI batch simulation with equal-length sessions,
    use compute_summary_stats with 2D arrays instead.
    
    Args:
        session_data: List of dicts, each with 'choices', 'stimuli', 'categories'
                      (as returned by TrialData.get_model_arrays or FittingData.session_arrays)
        stat_names: Which stats to compute (default: DEFAULT_STATS)
        return_dict: If True, return list of dicts; if False, list of 1D arrays
    ...
    for sa in session_data:
        choices = sa['choices'] if isinstance(sa, dict) else sa[0]
        stimuli = sa['stimuli'] if isinstance(sa, dict) else sa[1]
        categories = sa['categories'] if isinstance(sa, dict) else sa[2]
        
        result = compute_summary_stats(
            choices, stimuli, categories,
            stat_names=stat_names, return_dict=return_dict
        )
        results.append(result)
