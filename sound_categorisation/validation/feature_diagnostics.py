...
# sigma, lapse_low, lapse_high) plus one pseudo-producer, 'update_matrix', whose
# 64 columns (um_<cur>_<prev>) come from the readout, not the stat registry.
UM_KEY = 'update_matrix'
N_UM_BINS = 8


def list_available_stats() -> List[str]:
    """Producer names (the feature-selection vocabulary) plus the UM pseudo-producer."""
    return list(list_producers()) + [UM_KEY]


def get_stat_names_expanded(names: Sequence[str]) -> List[str]:
    """Producer names -> scalar column names, in order; UM_KEY -> um_i_j (row-major)."""
    producers = list_producers()
    out: List[str] = []
    for n in names:
        if n == UM_KEY:
            out += [f'um_{i}_{j}' for i in range(N_UM_BINS) for j in range(N_UM_BINS)]
        elif n in producers:
            out += list(producers[n])
        else:
            raise KeyError(f'unknown stat producer {n!r}')
    return out


def _stat_vector(choices, stimuli, categories, names: Sequence[str]) -> np.ndarray:
    """Simulated block -> float vector in ``get_stat_names_expanded(names)`` order."""
    arrays = TrialArrays.from_sequence(choices, stimuli, categories)
    scalars = [n for n in names if n != UM_KEY]
    parts = []
    if scalars:
        parts.append(compute_stats(arrays, get_stat_names_expanded(scalars), strict=False).to_numpy())
    if UM_KEY in names:
        parts.append(compute_update_matrix(arrays, n_bins=N_UM_BINS).matrix.ravel())
...
def compute_param_stat_correlations(model_type, stat_names=None, n_samples=1000,
    ...
    for _ in range(n_samples):
        ...
        stats = _stat_vector(choices, stimuli, categories, stat_names)
